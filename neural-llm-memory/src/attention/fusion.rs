...
mod tests {
    use super::*;
    use crate::graph::cross_modal::CrossModalConfig;
    
    #[test]
    fn test_attention_fusion_creation() {
        let cross_modal_config = CrossModalConfig::default();
        let cross_modal_bridge = Arc::new(CrossModalBridge::new(cross_modal_config));
        
        let config = AttentionFusionConfig::default();
        let fusion = AttentionFusion::new(config, cross_modal_bridge);
        
        let stats = fusion.get_stats();
        assert_eq!(stats.active_modalities, 8);
    ...
    fn test_multi_modal_fusion() {
        ...
        // Add memories for different modalities
        ...
        let active_modalities = vec![MemoryModality::Semantic, MemoryModality::Episodic];
        
        let result = fusion.fuse_attention(&query, memory_contents, active_modalities);
        assert!(result.is_ok());
        
        let fusion_result = result.unwrap();
        assert!(fusion_result.latency_ms < 5.0); // Should meet performance target
        assert_eq!(fusion_result.attention_weights.len(), 2);
        assert_eq!(fusion_result.modality_contributions.len(), 2);
    }
}