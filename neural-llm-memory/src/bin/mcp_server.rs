...
use neural_llm_memory::adaptive::{AdaptiveMemoryModule, AdaptiveConfig, enhanced_search, MatchType};
use neural_llm_memory::graph::{ConsciousGraph, ConsciousGraphConfig};
...
impl NeuralMemoryServer {
    async fn new() -> Result<Self> {
        ...
        // Initialize graph for enhanced search
        let graph = if adaptive_enabled {
            eprintln!("🔗 Initializing knowledge graph...");
            match ConsciousGraph::new_with_config(ConsciousGraphConfig::default()) {
                Ok(g) => {
                    eprintln!("✅ Knowledge graph initialized successfully");
                    Some(Arc::new(g))
                },
                Err(e) => {
                    eprintln!("⚠️  Failed to initialize graph: {}. Enhanced search disabled.", e);
                    None
                }
            }
        } else {
    ...
    async fn search_memory(
        ...
        if self.adaptive_enabled {
            ...
            if params.include_related && self.graph.is_some() {
                // Use enhanced search with graph traversal
                let graph = self.graph.as_ref().unwrap();
                let result = enhanced_search(
                    &*module,
                    &graph,
                    &params.query,
                    params.limit,
                ...
                // Format matches with relationship info
                let matches: Vec<_> = result.matches.into_iter()
                    .map(|m| {
                        ...
                        if let Some(rel) = m.relationship {
                            match_obj["relationship"] = json!({
                                "type": rel.edge_type,
                                "from": rel.path_from,
                ...
                // Regular search without graph traversal
