...
mod tests {
    ...
    fn test_emotional_memory_integration() {
        ...
        let content = Array1::from_vec(vec![0.1, 0.2, 0.3]);
        let joy_state = EmotionalState::from_emotion(Emotion::Joy, 0.8);
        
        emotional_memory.store_with_emotion("happy_memory".to_string(), content, joy_state).unwrap();
        
        // Check emotional tag
        let retrieved_emotion = emotional_memory.get_emotional_context("happy_memory");
        assert!(retrieved_emotion.is_some());
        assert!(retrieved_emotion.unwrap().valence > 0.0);
    }
    ...
    fn test_mood_congruent_retrieval() {
        ...
        let content1 = Array1::from_vec(vec![0.1, 0.2, 0.3]);
        let content2 = Array1::from_vec(vec![0.2, 0.3, 0.4]);
        ...
        emotional_memory.store_with_emotion(
            "sad".to_string(), 
            content2, 
            EmotionalState::from_emotion(Emotion::Sadness, 0.9)
        ).unwrap();
        
        // Query in happy mood
        let query = Array1::from_vec(vec![0.15, 0.25, 0.35]);
        let happy_mood = EmotionalState::from_emotion(Emotion::Joy, 0.7);
        
        let results = emotional_memory.retrieve_mood_congruent(&query, &happy_mood, 1);
        
        // Should prefer the happy memory
        assert_eq!(results[0].0, "happy");
    }
}
