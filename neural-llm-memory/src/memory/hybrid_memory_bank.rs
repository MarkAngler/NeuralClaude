...
mod tests {
    ...
    #[test]
    fn test_hybrid_memory_bank_creation() {
        let bank = HybridMemoryBank::new(100, 10, HybridMemoryConfig::default());
        assert_eq!(bank.size(), 0);
    }
    
    #[test]
    fn test_consciousness_aware_storage() {
        let mut bank = HybridMemoryBank::new(100, 10, HybridMemoryConfig::default());
        
        let key = MemoryKey::from_id("test_key");
        ...
        // Set high consciousness context
        let context = ConsciousnessContext {
            ...
            metacognitive_confidence: 0.85,
        };
        
        bank.store_with_consciousness(key.clone(), value, Some(context)).unwrap();
        
        // Verify tags were added
        let retrieved = bank.retrieve(&key).unwrap().unwrap();
        assert!(retrieved.metadata.tags.contains(&"consciousness_level".to_string()));
        assert!(retrieved.metadata.tags.contains(&"emotion".to_string()));
        assert!(retrieved.metadata.tags.contains(&"self-related".to_string()));
    }
    
    #[test]
    fn test_emotional_consolidation() {
        let mut bank = HybridMemoryBank::new(100, 10, HybridMemoryConfig::default());
        
        // Store memories with different emotions
        for i in 0..5 {
            let key = MemoryKey::from_id(&format!("joy_{}", i));
            let mut value = MemoryValue {
                embedding: vec![0.1; 768],
                content: format!("Joyful memory {}", i),
                metadata: MemoryMetadata::default(),
            };
            value.metadata.tags.push("emotion".to_string());
            value.metadata.tags.push("joy".to_string());
            bank.store(key, value).unwrap();
        }
        
        for i in 0..3 {
            let key = MemoryKey::from_id(&format!("sad_{}", i));
            let mut value = MemoryValue {
                embedding: vec![0.2; 768],
                content: format!("Sad memory {}", i),
                metadata: MemoryMetadata::default(),
            };
            value.metadata.tags.push("emotion".to_string());
            value.metadata.tags.push("sadness".to_string());
            bank.store(key, value).unwrap();
        }
        
        // Set emotional context to joy
        let context = ConsciousnessContext {
        ...
        bank.update_consciousness_context(context);
        
        // Perform consolidation
        bank.consolidate_with_emotion().unwrap();
        
        // Verify joyful memories got boosted
        let joy_memory = bank.retrieve(&MemoryKey::from_id("joy_0")).unwrap().unwrap();
        assert!(joy_memory.metadata.importance > 0.5);
    }
    
    #[test]
    fn test_consciousness_weighted_search() {
        let mut bank = HybridMemoryBank::new(100, 10, HybridMemoryConfig::default());
        
        // Store memories with different consciousness levels
        for i in 0..5 {
            let key = MemoryKey::from_id(&format!("memory_{}", i));
            let mut value = MemoryValue {
            ...
            value.metadata.tags.push(
                (0.2 * i as f32).to_string(),
            );
            
            bank.store(key, value).unwrap();
        }
        
        // Search with high consciousness context
        let query = Array2::from_shape_vec((1, 768), vec![0.3; 768]).unwrap();
        let context = ConsciousnessContext {
            consciousness_level: 0.9,
            emotional_state: EmotionalContext {
                valence: 0.0,
        ...
        let results = bank.search_with_consciousness(&query, 3, Some(context));
        
        assert_eq!(results.len(), 3);
        // Results should be weighted by consciousness
    }
}