...
mod tests {
    ...
    fn test_scaled_dot_product_attention() {
        ...
        let query = arr2(&[[1.0, 0.0], [0.0, 1.0]]);
        let key = arr2(&[[1.0, 0.0], [0.0, 1.0]]);
        let value = arr2(&[[1.0, 2.0], [3.0, 4.0]]);
        
        let result = attention.scaled_dot_product_attention(&query, &key, &value, None);
        
        assert_eq!(result.shape(), &[2, 2]);
        // Result should be close to value matrix due to identity attention pattern
    }
    ...
    fn test_softmax_1d() {
        ...
        attention.softmax_1d_inplace(&mut data);
    ...
    fn test_multi_head_attention() {
        ...
        let query = Array2::from_shape_fn((seq_len, d_model), |(i, j)| (i + j) as f32 * 0.1);
        let key = query.clone();
        let value = Array2::from_shape_fn((seq_len, d_model), |(i, j)| (i * j) as f32 * 0.1);
        
        let result = attention.multi_head_attention(&query, &key, &value, num_heads, d_model);
    ...
    fn test_attention_with_cache() {
        ...
        let result = attention.compute_attention_with_cache(&query, &key_cache, &value_cache);
        
        assert_eq!(result.len(), 2);
        // First value should have higher weight due to matching key
    ...
    fn test_matmul_simd() {
        ...
        let result = attention.matmul_simd(&a, &b);
