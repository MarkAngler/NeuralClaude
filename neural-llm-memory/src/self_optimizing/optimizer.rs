...
mod tests {
    ...
    fn test_online_optimizer_creation() {
        ...
        let strategy = AdaptationStrategy::default();
        let optimizer = OnlineOptimizer::new(network, strategy);
        
        assert_eq!(optimizer.history.adaptations.len(), 0);
    }
    ...
    fn test_performance_monitor() {
        ...
        // Add some data
        for i in 0..20 {
            monitor.update_train_loss(1.0 - 0.01 * i as f32);
            monitor.update_val_loss(1.0 - 0.005 * i as f32);
            monitor.update_gradient_norm(0.1);
        }
        
        // Should not detect vanishing gradients
        assert!(!monitor.detect_vanishing_gradients());
        
        // Should not detect overfitting (val loss decreasing)
        assert!(!monitor.detect_overfitting());
        
        // Add plateau data
        for _ in 0..30 {
            monitor.update_val_loss(0.9);
        }
        
        // Should detect plateau
        assert!(monitor.detect_plateau());
    }
    ...
    fn test_lr_scheduler() {
        let mut scheduler = AdaptiveLRScheduler::new(
            0.1,
            LRSchedulerType::CosineAnnealing { total_steps: 100 }
        );
        ...
        // Should be near zero at the end
